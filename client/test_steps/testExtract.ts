...
async function extractInfo(userInput: string, config: RunnableConfig): Promise<{ userId: string; title: string; author: string }> {
    const prompt = `
...
// テスト実行
const userInput =
  "私はUserId1のユーザーです。村上春樹作『ノルウェイの森』の書評をお願いします。";
extractInfo(userInput, {});
